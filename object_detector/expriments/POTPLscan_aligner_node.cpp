// scan_alignment_node.cpp  (Point-to-Plane ICP only, with planar option)
// - MapManagerPair가 sensor_msgs::msg::PointCloud2::SharedPtr를 저장/반환한다고 가정
// - 과거 스캔: 오도메 1차 정렬 → Pt2Pl ICP 2차 보정
// - /aligned_history_scans (odometry) /aligned_history_scans_icp (Pt2Pl 결과) 퍼블리시
...
#include "icp_pt2pl_refiner.hpp"      // 위 헤더

using std::placeholders::_1;
using std::placeholders::_2;

class ScanAlignmentNode : public rclcpp::Node {
public:
  ...
  ScanAlignmentNode() : Node("scan_alignment_node"), map_manager_(10)
  {
    ...
    sync_->registerCallback(std::bind(&ScanAlignmentNode::syncCallback, this, _1, _2));
    ...
    // ======= Pt2Pl 파라미터 (런타임 override 가능) =======
    this->declare_parameter<int>("pt2pl.max_iterations", 30);
    this->declare_parameter<double>("pt2pl.max_corr_dist", 0.5);
    this->declare_parameter<double>("pt2pl.trans_eps", 1e-6);
    this->declare_parameter<double>("pt2pl.fit_eps", 1e-5);
    this->declare_parameter<double>("pt2pl.voxel_leaf", 0.10);
    this->declare_parameter<bool>("pt2pl.use_downsample", true);
    this->declare_parameter<bool>("pt2pl.reject_far", false);
    this->declare_parameter<double>("pt2pl.reject_radius", 3.0);
    // 법선 추정(2D LaserScan이면 KNN 권장, 혹은 planar 고정)
    this->declare_parameter<bool>("pt2pl.normal_use_radius", false);
    this->declare_parameter<double>("pt2pl.normal_radius", 0.3);
    this->declare_parameter<int>("pt2pl.normal_k", 20);
    // 2D 평면 가정 & 최소 포인트 수
    this->declare_parameter<bool>("pt2pl.assume_planar", true);
    this->declare_parameter<int>("pt2pl.min_points", 10);

    updateFromParams();

    RCLCPP_INFO(this->get_logger(), "scan_alignment_node (Point-to-Plane ICP) started.");
  }
...
private:
  ...
  void updateFromParams() {
    Pt2PlParams pp;
    pp.max_iterations     = this->get_parameter("pt2pl.max_iterations").as_int();
    pp.max_corr_dist      = static_cast<float>(this->get_parameter("pt2pl.max_corr_dist").as_double());
    pp.trans_eps          = this->get_parameter("pt2pl.trans_eps").as_double();
    pp.fit_eps            = this->get_parameter("pt2pl.fit_eps").as_double();
    pp.voxel_leaf         = static_cast<float>(this->get_parameter("pt2pl.voxel_leaf").as_double());
    pp.use_downsample     = this->get_parameter("pt2pl.use_downsample").as_bool();
    pp.reject_far         = this->get_parameter("pt2pl.reject_far").as_bool();
    pp.reject_radius      = static_cast<float>(this->get_parameter("pt2pl.reject_radius").as_double());
    pp.normal_use_radius  = this->get_parameter("pt2pl.normal_use_radius").as_bool();
    pp.normal_radius      = static_cast<float>(this->get_parameter("pt2pl.normal_radius").as_double());
    pp.normal_k           = this->get_parameter("pt2pl.normal_k").as_int();
    pp.assume_planar      = this->get_parameter("pt2pl.assume_planar").as_bool();
    pp.min_points         = this->get_parameter("pt2pl.min_points").as_int();
    refiner_ = IcpPointToPlaneRefiner(pp);
  }
                    ...
                    const nav_msgs::msg::Odometry::ConstSharedPtr& odom_msg)
  {
    ...
    pcl::PointCloud<PXYZI> all_aligned_icp;  // 여기서는 Pt2Pl 결과
    ...
    for (size_t i = 0; i < frames; ++i) {
      ...
      if (hist_in_curr.size() < 10) continue; // 최소 포인트 가드
      ...
      Eigen::Matrix4f dT = refiner_.refine(
    ...
    if (fitness_cnt > 0) {
      ...
      RCLCPP_INFO_THROTTLE(
        ...
        "[Pt2Pl] mean fitness=%.6f over %d pairs", mean_fit, fitness_cnt
      );
    }
  }
  ...
  IcpPointToPlaneRefiner refiner_;
};

int main(int argc, char **argv)
{
  rclcpp::init(argc, argv);
  rclcpp::spin(std::make_shared<ScanAlignmentNode>());
  rclcpp::shutdown();
  return 0;
}
